  ...
  return (
    <div
      style={{
        ...
        padding: "24px",
        boxShadow: "0 4px 20px rgba(26,107,58,0.04)",
        display: "flex",
    ...
    >
      ...
      <div>
        ...
        {sub && (
          <div style={{ fontSize: "11px", color, fontWeight: 600, marginTop: "4px" }}>
            {sub}
          </div>
        )}
      </div>
    ...
    >
      <div
        style={{
          ...
          marginBottom: "24px",
          paddingBottom: "16px",
          borderBottom: "1px solid rgba(26,107,58,0.06)",
        }}
      >
        <div
          style={{
    ...
    <div>
      ...
      <style>{`
        ...
        .analytics-activity-row {
          display: grid;
          grid-template-columns: 1fr 64px 72px 64px;
          gap: 8px;
          padding: 12px 0;
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "15px", color: "var(--muted)", marginTop: "6px", margin: 0 }}>
            Estadístiques d&apos;ús de la web de GironaXics
            {lastUpdated && (
      ...
      <div className="analytics-filter-row" style={{ marginBottom: "24px" }}>
        {PERIOD_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            className={`analytics-period-btn ${days === opt.value ? "active" : ""}`}
      ...
      {data && (
        <>
          ...
          <div className="analytics-kpi-grid">
            <KpiCard
              icon={TrendingUp}
              label="Visites a activitats"
              value={data.totals.activityViews}
              color="#1a6b3a"
            />
            <KpiCard
              icon={Phone}
            ...
            <KpiCard
              ...
              value={data.totals.sponsorClicks}
              color="#d97706"
              sub={data.totals.casalsBannerClicks > 0 ? `+ ${data.totals.casalsBannerClicks} al banner casals` : undefined}
            />
          </div>
          ...
          <div className="analytics-section-grid">
            ...
            {/* Top activitats */}
            <SectionCard title="Activitats més vistes" icon={Star}>
              {data.topActivitats.length === 0 ? (
                <p style={{ color: "var(--muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>
                  Sense dades per aquest període
                </p>
              ) : (
                <>
                  <div
                    className="analytics-activity-row"
                    style={{ fontWeight: 700, color: "var(--muted)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.05em", paddingTop: 0 }}
                  >
                    <span>Activitat</span>
                  ...
                  {data.topActivitats.map((a, i) => (
                    <div key={i} className="analytics-activity-row">
                      ...
                      <span style={{ textAlign: "right", fontWeight: 700, color: "var(--verd-fosc)" }}>
                        {a.views}
                      </span>
                      ...
                      <span
                        style={{
                          textAlign: "right",
                          fontWeight: 700,
                          color: a.ratio >= 20 ? "#16a34a" : a.ratio >= 5 ? "#d97706" : "var(--muted)",
                        }}
                      >
                        {a.ratio}%
                      </span>
            ...
            <SectionCard title="Barris més filtrats" icon={MapPin}>
              {data.topBarris.length === 0 ? (
                <p style={{ color: "var(--muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>
                  Sense dades per aquest període
                </p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  {data.topBarris.map((b, i) => (
                    <HorizontalBar
                      key={i}
                      label={b.label}
                      count={b.count}
                      max={data.topBarris[0]?.count ?? 1}
                      color="#2563eb"
                    />
                  ))}
                </div>
            ...
            <SectionCard title="Franges d'edat més cercades" icon={Users}>
              {data.topEdats.length === 0 ? (
                <p style={{ color: "var(--muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>
                  Sense dades per aquest període
                </p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  {data.topEdats.map((e, i) => (
                    <HorizontalBar
                      key={i}
                      label={e.label}
                      count={e.count}
                      max={data.topEdats[0]?.count ?? 1}
                      color="#7c3aed"
                    />
                  ))}
                </div>
            ...
            <SectionCard title="Clics als sponsors" icon={MousePointerClick}>
              {data.topSponsors.length === 0 ? (
                <p style={{ color: "var(--muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>
                  Sense clics a sponsors en aquest període
                </p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  {data.topSponsors.map((s, i) => (
                    <HorizontalBar
                      key={i}
                      label={s.label}
                      count={s.count}
                      max={data.topSponsors[0]?.count ?? 1}
                      color="#d97706"
                    />
                  ))}
                  <div
                    style={{
                      marginTop: "8px",
                      paddingTop: "16px",
                      borderTop: "1px solid rgba(26,107,58,0.06)",
                      fontSize: "13px",
                      color: "var(--muted)",
                      display: "flex",
                      justifyContent: "space-between",
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 700, color: "var(--verd-fosc)" }}>
                      {data.totals.casalsBannerClicks} clics
                    </span>
                  </div>
                </div>
              )}
            ...
            <SectionCard title="Dispositius" icon={BarChart2}>
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div>
                  ...
                  >
                    <span style={{ fontWeight: 600 }}>📱 Mòbil</span>
                    <span style={{ fontWeight: 700, color: "var(--verd-fosc)" }}>
                      {data.devices.mobile} ({mobilePct}%)
                    </span>
                  </div>
                  <div
                    style={{
                ...
                <div>
                  ...
                  >
                    <span style={{ fontWeight: 600 }}>🖥️ Escriptori</span>
                    <span style={{ fontWeight: 700, color: "var(--verd-fosc)" }}>
                      {data.devices.desktop} ({100 - mobilePct}%)
                    </span>
                  </div>
                  <div
                    style={{
                ...
                <div
                  style={{
                    padding: "12px 16px",
                    backgroundColor: "rgba(26,107,58,0.03)",
                    borderRadius: "8px",
                    fontSize: "13px",
                    color: "var(--muted)",
                    textAlign: "center",
          ...
          >
            ...
            Els dispositius dels clics de filtres es detecten per mida de pantalla.
          </div>
        </>
      )}
    </div>
